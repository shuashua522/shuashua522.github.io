...
1. **安装** **gh-pages** **工具（用于推送** **dist** **到** **gh-pages** **分支）**
...
1. **样式错乱 / 图片加载失败**：检查 config.ts 里的 base 是否配置正确（必须和仓库路径一致，结尾带 /）。

1. **工作流运行失败**：打开「Actions」查看错误日志，常见原因是 base 配置错误、依赖安装失败（可重新推一次代码触发工作流）。

1. **404 页面**：确认 gh-pages 分支是否存在，以及「Pages」设置的分支是否正确。
